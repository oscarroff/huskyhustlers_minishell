...
/*   Updated: 2025/11/03 19:14:24 by thblack-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	vec_insert(t_vec *dst, void *src, size_t index)
{
	if (index > dst->len || !dst || !dst->data || !src)
		return (-1);
	if (dst->len + 1 >= dst->capacity / dst->elem_size)
		vec_resize(dst, dst->capacity * 2);
...
int	vec_remove(t_vec *src, size_t index)
{
	...
	src->len--;
	if (src->len < (src->capacity / src->elem_size) / 2)
		if (vec_resize(src, src->capacity / 2) < 0)
			return (-1);
	return (1);
}
